<?php
class ControllerPaymentTechProcessCard extends Controller {
	protected function index() {
		...
		$this->data['liveid'] = $this->config->get('techprocesscard_account');
		$this->data['testid'] = $this->config->get('techprocesscard_testaccount');
		$this->data['secret'] = $this->config->get('techprocesscard_secret');
		$this->data['responseurl'] = $this->config->get('techprocesscard_responseurl');
		$this->data['bank'] = $this->config->get('techprocesscard_bank');
...
*/
		if ($this->config->get('techprocesscard_test')) {
			$this->data['demo'] = 'Y';
		} else {
			$this->data['demo'] = '';
		}
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/techprocesscard.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/techprocesscard.tpl';
		} else {
			$this->template = 'default/template/payment/techprocesscard.tpl';
		}	
	...
	public function callback() {
		...
		if (strtoupper(md5($this->config->get('techprocesscard_secret') . $this->config->get('techprocesscard_account') . $order_number . $this->request->post['total'])) == $this->request->post['key']) {
			if ($this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false) == $this->request->post['total']) {
				$this->model_checkout_order->confirm($this->request->post['cart_order_id'], $this->config->get('techprocesscard_order_status_id'));
			} else {
