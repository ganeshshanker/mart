...
public function getManufacturers($data = array()) {
	...
	public function getCatManufacturers($data) {
		if($data!=0){
			$sql = "SELECT * FROM " . DB_PREFIX . "manufacturer m LEFT JOIN " . DB_PREFIX . "manufacturer_to_store m2s ON (m.manufacturer_id = m2s.manufacturer_id) WHERE m2s.store_id = " . (int)$this->config->get('config_store_id') . " AND m.manufacturer_id IN (SELECT manufacturer_id FROM " . DB_PREFIX . "product WHERE product_id IN (Select product_id from " . DB_PREFIX . "product_to_category WHERE category_id = " . (int)$data . ") GROUP By manufacturer_id) ORDER by m.name ASC";
		} else {
			$sql = "SELECT * FROM " . DB_PREFIX . "manufacturer m LEFT JOIN " . DB_PREFIX . "manufacturer_to_store m2s ON (m.manufacturer_id = m2s.manufacturer_id) WHERE m2s.store_id = " . (int)$this->config->get('config_store_id') . " ORDER by m.name ASC";
		}
