...
class ModelCatalogProduct extends Model {
				...
				foreach ($words as $word){
					...
					if (!empty($data['filter_name'])) {					
						if (!empty($data['filter_description'])) {
							$sql .= "LCASE(pd.name) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%' OR MATCH(pd.description) AGAINST('" . $this->db->escape(utf8_strtolower($word)) . "')";
						} else {
							$sql .= "LCASE(pd.name) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%'";
						}
					}
					
					if (!empty($word) && !empty($data['filter_tag'])) {
						$sql .= " OR ";
					}
	...
	public function getFeaturedProducts($limit) {
		...
		if (!$product_data) { 
			$query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' GROUP BY p.product_id ORDER BY RAND() LIMIT " . (int)$limit);
